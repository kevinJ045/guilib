	...
	if (status == "TAGW") {
	  if (c == "&") {
	    if (lookAhead(x, i, 7) == "&LT;!--") {
	      text += "<span style='color:#FFC66D'>" + c;
	      status = "COM";
	      continue;
	    }
	    if (lookAhead(x, i, 4) == "&LT;") {
	      ...
	      text += "<span style='color:#A9B7C6'>" + c;
	      status = "LT";
	      continue;
	    }
	  }
	  if (c == "{") {
	    if (lookAhead(x, i, 2) == "{{") {
	      text += "<span style='color:#FF9E59'>" + c;
	      status = "ANG";
	      continue;
	    }
	  }
	...
	if (status == "ELE") {
	  ...
	  if (c == "&") {
	    if (lookAhead(x, i, 4) == "&GT;") {
	      text += "</span><span style='color:#A9B7C6'>" + c;
	      status = "GT";
	      continue;
	    }
	  }
	...
	if (status == "ATT") {
	  if (c == "=") {
	    text += c + "</span><span style='color:#6A8759'>";
	    status = "VALW";
	    continue;
	  }
	  if (c == "&") {
	    if (lookAhead(x, i, 4) == "&GT;") {
	      text += "</span><span style='color:#6A8759'>" + c;
	      status = "GT";
	      continue;
	    }
	  }
	...
	if (status == "COLON") {
	  if (c == '"') {
	    status = "VALUE";
	    text += "<span style='color:#6A8759'>" + c;
	    continue;
	  }
	  if (ch > 47 && ch < 58) {
	    status = "NUMBER";
	...
	jsArr = ["var", "const", "let","boolean","break","case","catch","continue","debugger","default","do","else","finally","for",
	...
	for (i = 0; i < x.length; i++) {
	  ...
	  if (ele) {
	    ...
	    if (ele =="true" || ele == "false" || ele == "null" || isNaN(ele) == false || ele == "R921") {
	      text += "<span style='color:#6A8759'>" + x.substr(i,ele.length) + "</span>";
	      i += ele.length - 1;
	      status = "";
	      continue;
	    }
	    ...
	    for (k = 0; k < jsArr.length; k++) {
	      if (ele == jsArr[k]) {
	        text += "<span style='color:#FF9E59'>" + x.substr(i,ele.length) + "</span>";
	        i += ele.length - 1;
	        status = "SPW";
	        break;
	      }
	  ...
	  text = text.replace(/R921/imgs,"<span style='color:#FF9E59'>R921</span>");
	  text = text.replace(/\$/imgs,"<span style='color:#FF9E59'>$</span>");
