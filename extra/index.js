...
var parseEasingParameters = function(string) {
  var match2 = /\(([^)]+)\)/.exec(string);
  return match2 ? match2[1].split(",").map(function(p) {
    return parseFloat(p);
  }) : [];
};
...
var getUnit = function(val) {
  var split2 = /[+-]?\d*\.?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?(%|px|pt|em|rem|in|cm|mm|ex|ch|pc|vw|vh|vmin|vmax|deg|rad|turn)?$/.exec(val);
  if (split2) {
    return split2[1];
  }
};
...
class WidgetProps2 {
  ...
  clone(selectedOptions = {}) {
    let options2 = { cloneChildren: true, cloneEvents: false, ...selectedOptions };
    let el = findEl(this.id);
    let cloned = el.clone(true, true);
    let events2 = {};
    if (options2.cloneEvents) {
      this.__events__.forEach((evt) => {
        events2[evt.event] = evt.callback;
      });
    }
    let opts = { ...this.options, children: [], ...options2 };
    if (!Array.isArray(opts.children))
      opts.children = [];
    let children = options2.cloneChildren == "options" ? this.options.children : this.children();
    if (children && options2.cloneChildren) {
      opts.children = children.map((item) => item.clone({ cloneChildren: options2.cloneChildren, cloneEvents: options2.cloneEvents })).concat(opts.children);
    }
...
function createWidgetModel(model, _options, widget = Widget_default) {
  const classGenerated = class extends widget {
    options = {};
    constructor(options4 = _options) {
      let wo = {};
      if (model.widgetOptions) {
        for (let i in model.widgetOptions)
          wo[i] = resolveValue(model.widgetOptions[i], { type: "list", value: options4 }).value;
      }
...
var initiateSetters = function(widget2, setterFunctions, options6) {
  setterFunctions.forEach((setterFull) => {
    ...
    if (options6[setter]) {
      widget2[name] = options6[setter];
    }
  });
};
var _init = function(widget2, options6) {
  let elementRaw;
  if (!widget2.__generated) {
    if (options6.element.raw || options6.element.selector) {
      elementRaw = new dom_default(options6.element.raw).at(0);
    } else {
      elementRaw = document.createElement(options6.element.name);
    }
  }
  const element = widget2.__generated ? findEl(widget2.id) : new dom_default(elementRaw);
  if (!widget2.__generated && options6.element.html)
    element.html(options6.element.html);
  if (widget2.__generated && options6.reset)
    element.attr({ class: "", style: "" });
  element.addClass(options6.class);
  if (options6.position) {
    let { type: type4, centered, top, left, right, bottom } = options6.position;
    element.css({
  ...
  if (options6.store instanceof Store_default)
    widget2.store = options6.store;
  if (typeof options6.build == "function") {
    if (!options6.children)
      options6.children = [];
    let child2 = options6.build(options6, widget2);
    if (Array.isArray(child2)) {
      options6.children.push(child2);
    } else {
      options6.children.push(child2);
    }
  }
  if (options6.children && options6.children.length) {
    options6.children.forEach((element2) => {
      if (element2 == null)
        return;
  ...
  if (options6.private === true) {
    delete options6.private;
  }
  if (options6.accepts === false) {
    widget2.accepts = false;
  } else if (options6.accepts === true) {
    widget2.accepts = true;
  }
  for (var i in options6) {
    if (i.match(/on([A-Z])([a-zA-Z]+)/)) {
      if (!options6.events)
        options6.events = {};
      options6.events[i.replace("on", "").toLowerCase()] = options6[i];
    }
  }
  if (options6.events) {
    for (var i in options6.events) {
      widget2.on(i, options6.events[i]);
    }
  }
  if (options6.props) {
    element.prop(options6.props);
  }
  if (options6.attr) {
    element.attr(options6.attr);
  }
  widget2.options = widget2.registerProxy(options6, () => {
    widget2.setOptions({});
  });
  let setters = [...setterFunctions];
  if (options6._setters) {
    setters.push(...options6._setters);
  }
  initiateSetters(widget2, setters, options6);
  if (!widget2.__generated)
...
var getAllSetters = function(obj) {
  ...
  while (currentObj) {
    ...
    for (const key in descriptors) {
      ...
      if (key == "__proto__")
        continue;
      if (setterFunctions.includes(key) || setterFunctions.find((k) => k.endsWith(":" + key)))
        continue;
      if (descriptor.set) {
        setters.push(key);
      }
  ...
  return function(target, extended) {
    ...
    class WidgetClass extends target {
      constructor(options6) {
        let o = mergeOptions({ ...defaults, _setters: setters }, options6);
        super(o);
        this.onInit(o);
      }
      onInit(o) {
...
class Widget6 extends WidgetProps_default {
  ...
  constructor(options6) {
    ...
    if (typeof options6 !== "object")
      options6 = { element: { name: "div" }, class: "widget" };
    _init(this, { ...getDefaults({}), ...options6 });
  }
  setOptions(options6) {
    const currentOptions = { ...this.options, ...options6 };
...
class Style2 extends WidgetEventTarget {
  name = "";
  values = {};
  ...
  extends(props, override = true) {
    return Style2.extends(this, props, this.name, override);
  }
  ...
  static fromWidget(widget2, name = null) {
  ...
  static copy(target, name = null, mode = "all", style = null, update = true) {
    if (mode == "all" && target instanceof Style2) {
      let newStyle = style || new Style2(name || id_default(5));
      ...
      return Style2.register(name || id_default(5), all);
    }
  }
  static extends(target, props, name = null, override = true) {
    let style = new Style2(name);
    Style2.copy(target, name, "all", style, override);
    if (!override)
      target.on("update", () => {
...
class ListBuilder extends Widget_default {
  ...
  updateList(newOptions) {
    ...
    const options6 = { ...this.options, ...newOptions };
    if (options6.items) {
      const doItems = () => {
      ...
      if (options6.items instanceof Promise) {
        options6.items.then((items) => {
          options6.items = items;
          doItems();
        });
      } else {
        doItems();
    ...
    if (options6.loader) {
      super.add(options6.loader);
    }
    if (options6.loading) {
      options6.loader?.show();
    } else {
      options6.loader?.hide();
    }
    if (typeof this._onUpdate == "function") {
      this._onUpdate(options6);
    }
    return this;
  }
  _onUpdate(any) {
...
function asyncComponent(options7) {
  return function(constructor, extended) {
    ...
    constructor.prototype.build = function(...args) {
      let mainWidget = new Widget_default({
        children: [options7?.loading?.() || null]
      });
...
class ComponentStyles {
  ...
  set(name, styles) {
    ...
    this.styles[n] = new Style_default(n, styles);
    return this;
  }
  get(name) {
    const n = this.id + "_" + name;
...
// client/utils/extra.ts
function getComponentExports() {
  return window.__rayous_exports;
}
var export_Macy = Macy.default;

export {
  voca_default as voca,
  uiwidget,
  typeref,
  ref,
  options,
