...
class WidgetClass {
	...
	addHTMLElement(child, subchild){
		...
		const elt = this.add(hadGUI ? child.GUIWIDGET : WidgetClass.from(child), subchild);
	...
	parent(){
		...
		if(!parent) return WidgetClass.from(document.body);
		...
		if(state.$children){
			state.$children = state.$children.map(element => {
				if(isWidget(element)){
					...
					return WidgetClass.from(element);
				}
			});
  ...
  getState() {
	...
	*/
	...
	static from(raw){
		return $(raw)[0].GUIWIDGET ?  $(raw)[0].GUIWIDGET : new WidgetClass({
			element: { raw: $(raw)[0] }
		});
	}
...
function AssemblyWidget(Func, el){
	const widget = WidgetClass.from(el);
